...
class MembershipPaymentTests(unittest.TestCase):
    ...
    def test_member_name_input_mode_follows_quickbooks_confirmation(self):
        ...
        for payment_option in (
            ...
            with self.subTest(payment_option=payment_option):
                self.assertEqual(
                    member_name_input_mode(payment_option, None),
                    "choice",
                )
                self.assertEqual(
                    member_name_input_mode(payment_option, True),
                    "quickbooks",
                )
                self.assertEqual(
    ...
    def test_non_quickbooks_plan_continues_without_collecting_a_name(self):
        ...
        self.assertTrue(
            all(
                line["name"] == ""
                for line in build_membership_lines([payment], handling_mode="automatic")
            )
        )
...
""",
    ...
    def test_stale_success_status_cannot_unlock_current_mismatch_downloads(self):
        ...
        runtime_root = Path(__file__).parent / f"_stale_status_{uuid4().hex}"
        input_dir = runtime_root / "input"
        iif_dir = runtime_root / "qb_imports"
        log_dir = runtime_root / "logs"
        temp_dir = runtime_root / "runtime"
        for folder in (input_dir, iif_dir, log_dir, temp_dir):
            folder.mkdir(parents=True, exist_ok=True)
        (log_dir / "last_run_status.txt").write_text(
            """HWFC Daily Deposit — Monday, September 14, 2026
  SALES: ✓ MATCH   $86,502.80
  DISCOUNTS: ✓ MATCH   $3,981.21
  HASH SALES: ✓ MATCH   $10.89
  ✓ ALL CHECKS PASSED — Safe to import into QuickBooks!
""",
        ...
        class FakeSubprocess:
            def run(self, _command, **_kwargs):
                (iif_dir / "deposit_20260914.iif").write_bytes(b"current iif")
                return SimpleNamespace(
                    stdout="SALES CHECK\nMISMATCH current run",
                    stderr="",
                    returncode=0,
                )
        ...
        self.assertFalse(result["validation"]["all_ok"])
        self.assertNotIn("ALL CHECKS PASSED", result["log_text"])
